...
deer = mesh.build(Patcher.mesh2meta("models/deer_small.1.node", relations=["cv", "ev", "ce"]))

x = deer.verts.x
v = deer.verts.v
f = deer.verts.f
m = deer.verts.m
mul_ans = deer.verts.mul_ans
...
b = ti.Vector.field(3, dtype=ti.f32, shape=len(deer.verts))
r0 = ti.Vector.field(3, dtype=ti.f32, shape=len(deer.verts))
p0 = ti.Vector.field(3, dtype=ti.f32, shape=len(deer.verts))
y = ti.Vector.field(3, dtype=ti.f32, shape=len(deer.verts))
x0 = ti.Vector.field(3, dtype=ti.f32, shape=len(deer.verts))
...
indices = ti.field(ti.u32, shape = len(deer.cells) * 4 * 3)
@ti.kernel
def init():
    for c in deer.cells:
        Ds = ti.Matrix.cols([c.verts[i].x - c.verts[3].x for i in ti.static(range(3))])
...
window = ti.ui.Window("Projective Dynamics", (1024, 768), vsync=True)
...
while window.running:
    ...
    scene.mesh(deer.verts.x, indices, color = (0.5, 0.5, 0.5))
